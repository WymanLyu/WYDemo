...
//        [_transitionFilter setDefaults];
        // 矢量对象640 × 960
        _vetor = [CIVector vectorWithX:0 Y:0 Z:640 W:960];
...
- (void)transition {
    ...
    // 设置属性
    [_transitionFilter setValue:@(640) forKey:@"inputWidth"];
    [_transitionFilter setValue:[CIColor colorWithCGColor:[UIColor blueColor].CGColor] forKey:@"inputColor"];
    ...
    // 设置裁剪区
    CIVector *cropVector = [CIVector vectorWithCGPoint:CGPointMake(20, 30)];
    cropVector = [cropVector initWithCGRect:CGRectMake(0, 0, 640, 960)];

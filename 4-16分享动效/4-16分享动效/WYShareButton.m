...
- (void)initPropertyWithFrame:(CGRect)frame {
    ...
    // 颜色
    self.lineLayerColor = [UIColor grayColor];
    self.lineLayerWidth = 2.0;
    // 小点
    self.midLayerW = 6;
    // 斜线/path
    CGFloat margin = 10;
...
- (void)select {
    ...
    aniScale2.toValue = [NSValue valueWithCGRect:bounds1];
    ...
    aniScale3.toValue = [NSValue valueWithCGRect:bounds1];
    ...
    aniContents1.fromValue = self.midContents1;
...
- (CALayer *)createCenterLayerWithCenterPoint:(CGPoint)centerP {
    ...
    midLayer.contents = (__bridge id _Nullable)([UIImage wy_imageWithUIColor:[UIColor grayColor] andFrame:midLayer.bounds].CGImage);
    return midLayer;
}

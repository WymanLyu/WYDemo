...
- (void)setupSubBtn {
    ...
    [self.sharePlatSubBtns removeAllObjects];
    ...
    for (int i = 0; i < self.scrollView.subviews.count; i++) {
        ...
        [itemBtn setAdjustsImageWhenHighlighted:NO];
        [self.sharePlatSubBtns addObject:itemBtn];
    }
    // 设置contentSize
...
- (void)shareItemBtnClick:(UIButton *)btn {
    ...
    if ([self.delegate respondsToSelector:@selector(shareViewItemPlatFormClick:)]) {
        [self.delegate shareViewItemPlatFormClick:btn.tag];
    }
...
- (NSMutableArray<UIButton *> *)sharePlatSubBtns {
    if (!_sharePlatSubBtns) {
        _sharePlatSubBtns = [NSMutableArray array];
    }
    return _sharePlatSubBtns;
}
